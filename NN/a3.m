function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  clf;
  hold on;
  plot(training_data_losses, "b;training;");
  plot(validation_data_losses, "r;validation;");
  ylabel("loss");
  ...
  for data_i = 1:3,
    ...
    if wd_coefficient!=0,
      printf("The classification loss (i.e. without weight decay) on the %s data is %f\n", data_name, loss(model, data, 0));
    end
    printf("The classification error rate on the %s data is %f\n", data_name, classification_performance(model, data));
  end
endfunction
...
function test_gradient(model, data, wd_coefficient)
 ...
 printf("Gradient test passed. That means that the gradient that your code computed is within 0.001%% of the gradient that the finite difference approximation computed, so the gradient calculation procedure is probably correct (not certainly, but probably).\n");
endfunction
...
function ret = loss(model, data, wd_coefficient)
  ...
  class_normalizer = log_sum_exp_over_rows(class_in); % log(sum(exp)) is what we subtract to get normalized log class probabilities. size: <1> by <number of data cases>
  log_class_prob = class_in - repmat(class_normalizer, [size(class_in, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
  class_out = exp(log_class_prob); % probability of each class. Each column (i.e. each case) sums to 1. size: <number of classes, i.e. 10> by <number of data cases>
  
  classification_loss = -mean(sum(log_class_prob .* data.outputs, 1)); % select the right log class probability using that sum; then take the mean over all data cases.
  wd_loss = sum(model_to_theta(model).^2)/2*wd_coefficient; % very straightforward: E = 1/2 * coefficient * theta^2
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  w = sum(exp(Z), 1);
  k = size(data.outputs, 1);
  w = repmat(w, k, 1);
  w = exp(Z) ./ w;
  E_by_a1 = (w-data.outputs) ;
...
function ret = theta_to_model(theta)
  ...
  ret.input_to_hid = reshape(theta(1: 256*n_hid), 256, n_hid).';
...
function ret = initial_model(n_hid)
  ...
  ret = theta_to_model(cos(0:(n_params-1))(:) * 0.1); % We don't use random initialization, for this assignment. This way, everybody will get the same results.
endfunction
